...
void DIIS::WriteVector(double * vector){
    ...
    // replace one.
    char * oldvector = (char*)malloc(100000*sizeof(char));
...
void DIIS::Extrapolate(double * vector){
    ...
    if (diis_iter_ <= maxdiis_){
        ...
        //jmax = 1; //have it replace oldest vector
        psio->close(PSIF_DCC_EVEC,1);
...
void DIIS::DIISCoefficients(int nvec){
    ...
    //outfile->Printf("\n");
    //outfile->Printf("    ==&gt DIIS Expansion Coefficients <==\n");
    //outfile->Printf("\n");
